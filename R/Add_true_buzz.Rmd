...
```{r}
write.csv(buzz_times, file = "E:/Myotis_vivesi/data/Audio/Mviv17/Mviv17_33/truebuzz/Mviv17_33_truebuzz.csv")
```
...
for(i in 1:length(na.omit(buzz_times$true_buzz))){
  buzz <- 
    which.min(abs(Metrics$datetime - (buzz_times$true_buzz[i]))) # why do we subtract buzz_times?
  true_start <- 
    which.min(abs(Metrics$datetime - (buzz_times$true_buzz[i] - 0.25))) # can we just say buzz -0.25?
  true_end <- 
