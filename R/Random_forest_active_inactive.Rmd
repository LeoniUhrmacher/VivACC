...
names(observations)[1] <- "behav"
observations$rel_Freq <- prop.table(observations$Freq)
table(val$behav) / nrow(val)


predictions <- table(val$pred)
names(predictions)[1] <- "behav"
predictions$rel_Freq <- prop.table(predictions$Freq)
table(val$pred) / nrow(val)
...
hist(dev$sd.dZ, xlim = c(0, 2), xlab = "sd.dZ", main = "", col = )


par(mfrow = c(1,2))
hist(dev$sd.dZ[which(dev$behav == "active")], xlim = c(0, 2), xlab = "sd.dZ", main = "active")

hist(dev$sd.dZ[which(dev$behav == "inactive")], xlim = c(0, 2), xlab = "sd.dZ", main = "inactive")
dev.off()
