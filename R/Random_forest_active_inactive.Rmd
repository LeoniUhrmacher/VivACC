...
text(Precision[which(names(Recall) %in% c("active", "inactive"))], Recall[which(names(Recall) %in% c("active", "inactive"))], names(Recall)[which(names(Recall) %in% c("active", "inactive"))], pos = 2, cex = 0.9)


save(dev, val, R.Forest, CM.dev, error.rate, min.OOB, var.Accuracy, GINI, sum_imp, CM.val, Precision, Recall, Accuracy, file = "E:/Myotis_vivesi/17_34_rm1/active_inactive/30_8_RF.Rdata")
...
# column =  observation, row = model prediction
CM.val <- as.data.frame(CM.val)
observations <- as.data.frame(table(val$behav))
predictions <- as.data.frame(table(val$pred))
...
rbind(CM.val, precision$Precision)

CM.val[6,] <- predictions$Freq
CM.val$observed <- observations$Freq
