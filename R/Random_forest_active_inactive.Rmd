...
# load Rdata
load(file = "E:/Myotis_vivesi/17_34_rm1/active_inactive/30_8_RF.Rdata")
...
mtry <- tuneRF(dev[,-1], dev$behav, ntreeTry = 22, mtryStart = 1, stepFactor = 0.5, improve = 0.05, trace = TRUE, plot = TRUE)
...
plot(Recall ~ Precision, pch = 16, bty = "L", las = 1, ylim = c(0,1), xlim = c(0.5, 1), cex = 1)
text(Precision, Recall, names(Recall), pos = 2, cex = 0.9)


save(dev, val, R.Forest, CM.dev, error.rate, min.OOB, var.Accuracy, GINI, CM.val, Precision, Recall, Accuracy, file = "E:/Myotis_vivesi/17_34_rm1/active_inactive/30_8_RF.Rdata")
...
boxplot(dev$behav, dev$sd.Z, ylab = "sd.Z", names = levels(dev$behav), col = c(2:3))

plot(dev$behav, dev$sd.Z, ylab = "sd.Z", xlab = "", col = c(2:3))

hist(dev$sd.Z, xlim = c(0, 2), xlab = "sd.Z", main = "inactive vs. active")


par(mfrow = c(1,2))
hist(dev$sd.Z[which(dev$behav == "inactive")], xlim = c(0, 1.5), xlab = "sd.Z", main = "inactive")

hist(dev$sd.Z[which(dev$behav == "active")], xlim = c(0, 1.5), xlab = "sd.Z", main = "active")
dev.off()
